...
To generate a similar project as by cloning the Git starter repository:
    ...
    mkdir vertx-wiki
    cd vertx-wiki
    mvn io.fabric8:vertx-maven-plugin:1.0.7:setup -DvertxVersion=3.4.1
    git init
...
Once it becomes evident in the sample code that callbacks do not always lead to readable code, we will introduce the RxJava support to show how the same asynchronous code can be better expressed by thinking in streams of processed events.
