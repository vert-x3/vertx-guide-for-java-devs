...
----
<1> SQL query results are being returned as instances of `JsonArray` and `JsonObject` instances.
...
----
<1> Form parameters sent through a HTTP POST request are avilable from the `RoutingContext` object. Note that without a `BodyHandler` within the `Router` configuration chain these values would not be available, and the form submission payload would need to be manually decoded from the HTTP POST request payload.
...
Since the build produces a Jar will all required dependencies embedded (including Vert.x and a JDBC database), running the wiki is as simple as:
    ...
    $ java -jar target/wiki-step-1-1.0-SNAPSHOT-fat.jar
