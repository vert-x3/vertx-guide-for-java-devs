...
The previous versions of this guide used callbacks. However we will be using __Future__ compositions in these examples.
...
When the _future_ of `prepareDatabase` completes successfully, then `startHttpServer` is called. When both steps complete; the promise passed as parameter to `start` method is marked complete or failed depending on the outcome.
`startHttpServer` is never called if `prepareDatabase` encounters an error and the overall initialization fails if either of the steps fails
...
The application verticle needs to keep a reference to a `JDBCPool` object (from the `io.vertx.jdbcclient` package) that serves as the connection pool to the database.
...
It uses a connection from the pool to perform a SQL query to create the `Pages` table unless it already exists:

[source,java,indent=0]
----
include::src/main/java/io/vertx/guides/wiki/MainVerticle.java[tags=prepareDatabase]
----
<1> `JDBCPool.pool` creates a connection pool by passing reference to the Vert.x instance and a configuration object
<2> The `JDBCPool` configuration is a JSON object. Here `url` is the JDBC URL.
...
<5> `JDBCPool.query()` uses the pool to create a sql query and `.execute()` to execute as an asynchronous operation. 
<6> `execute` returns a `Future<RowSet<Row>>` containing the results of the query. 
<7> When successful the `onSuccess` call back handler is invoked. We set the state of the `promise` to indicate that the operation succeded
<8> `onFailure` callback handler is called when the database initialization failed and the `promise` is set to indicate that the operation failed
<9> We complete the method by returning the _future_ associatted with the `promise`.
...
<4> __Important__ This makes all HTTP POST requests go through a first handler, here `io.vertx.ext.web.handler.BodyHandler`. This handler automatically decodes the body from the HTTP requests (e.g., form submissions), which can then be manipulated as Vert.x buffer objects.
<5> The router object can be used as a HTTP server handler, which then dispatches to other handlers as defined above.
<6> Starting a HTTP server is an asynchronous operation returning a `Future<HttpServer>`. Like the prepareDatabase callback handlers; the `onSuccess` and `onFailure` callback handlers set the state of the `promise` and return the _future_ associatted with it.
...
----
<1> SQL query to fetch all the pages is executed using the `JDBCPool` 
<2> We iterate over the fetched rows, extract and add data to a `JsonObject`. While the `RoutingContext` can be used to `put` arbitrary key / value data, we have avoided it.
<3> After assembly the `templateData` is passed to the `TemplateEngine` to render the page
<4> Finally the `onSuccess` callback handler is defined. We set the `Content-Type` and stream the rendered content as the HTTP response.
<5> In case of failure the `onFailure` is invoked. `RoutingContext` provides a sensible way to return a HTTP 500 error to the HTTP client.
...
Key / value data stored in the `templateData` JsonObject is made available as FreeMarker variable.
...
<2> Passing argument values to SQL queries is done using a `Tuple.of`, with the elements in order of the `?` symbols in the SQL query.
...
This happens both when updating an existing page (issuing a SQL `SQL_SAVE_PAGE` query) or saving a new page (issuing a SQL `SQL_CREATE_PAGE` query):
...
<3> The `JDBCPool` is used to create a SQL query based on the value of the hidden field. 
<4> Parameters are set on the query using `Tuple.of`. This happens both when updating an existing page or saving a new page
...
Since the build produces a Jar with all required dependencies embedded (including Vert.x and a JDBC database), running the wiki is as simple as:
    ...
    $ java -jar target/wiki-step-1-1.0.0-SNAPSHOT-fat.jar
