...
[NOTE]
====
Alternatively, the https://reactiverse.io[Reactiverse] community hosts a https://reactiverse.io/vertx-maven-plugin[Vert.x Maven plugin].
...
To generate a similar project as by cloning the Git starter repository:
    ...
    mkdir vertx-wiki
    cd vertx-wiki
    mvn io.reactiverse:vertx-maven-plugin:1.0.20:setup -DvertxVersion=3.6.0
    git init
