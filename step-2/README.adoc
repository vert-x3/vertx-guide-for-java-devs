...
<3> A class name as a string is also an option to specify a verticle to deploy. For other JVM languages string-based conventions allow a module / script to be specified.
<4> The `DeploymentOption` class allows to specify a number of parameters and especially the number of instances to deploy.
<5> A `CompositeFuture` is a future on top of other futures. Here the resulting future completes when both `httpVerticleDeployment` and `dbVerticleDeployment` complete, or fails when either of them fails.
<6> We define a handler that eventually completes the `MainVerticle` start future.
