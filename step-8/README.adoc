...
When direct queries do not fit (e.g. when using session variables/temporary tables), you can generate a reactive flow from a single `SQLConnection`:
...
You may execute any number of queries and transform results with RxJava operators to generate the flow.
