...
But it can become a bit tedious, especially when you combine different sources of events, or deal with complex data flows.
...
To acquire a database connection from the pool, you simply need to call `rxGetConnection` on the `JDBCClient`.
...
The method returns a `Single<Connection>` which you can easily transform to execute SQL queries with `flatMap`.
...
But how can we release the connection if the variable is no longer reachable?
...
<2> The `toSubscriber` method adapts the `resultHandler` to a `Subscriber<List<JsonObject>>`, so that the handler is invoked when the rows are emitted.
