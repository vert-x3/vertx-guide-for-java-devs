...
This guide is a gentle introduction to asynchronous programming with Vert.x, primarily aimed at developers familiar with mainstream non-asynchronous web development framework and libraries (e.g., Java EE, Spring).
We assume that the reader is familiar with the Java programming language and ecosystem.

We will start from a wiki web application backed by a relational database and server-side rendering of pages; then we will evolve it through several steps until it becomes a modern single-page application with _real-time web_ features.
...
2. Cleanly isolate each technical component as a reusable component unit called a _verticle_.
...
8. Refactoring some code to use reactive programing with the widely used RxJava library and its Vert.x integration.
9. Client-side programming with AngularJS.
...
Vert.x will let you access the Netty internals _if need be_, but in general you will better benefit from the higher-level APIs that Vert.x provides while not sacrifying performance compared to _raw_ Netty.
...
Vert.x is useful for a large range of applications: high volume message / event processing, microservices, API gateways, HTTP APIs for mobile applications, etc.
Vert.x and its ecosystem provide all sorts of technical tools for building end-to-end reactive applications.

While it may sound like Vert.x is only useful for demanding applications, the present guide also stresses that Vert.x also works very well for more traditional web applications.
As we will see, the code will remain relatively easy to comprehend, and should an application need to face a sudden peak in traffic then the code is already written with the essential ingredient for scaling up: _asynchronous processing of events_.

Finally, it is worth mentionning that Vert.x is _polyglot_ as it supports a wide range of popular JVM languages: Java, Groovy, Scala, Kotlin, JavaScript, Ruby and Ceylon.
The goal when supporting a language in Vert.x is not just to provide access to the APIs, but also to make sure that the language-specific APIs are idiomatic in each target language.
It is well-possible to develop different technical parts of an application with different JVM languages and the Vert.x APIs.
...
For instance suppose we have a verticle for dealing with HTTP requests, and a verticle for managing access to the database.
...
The event-bus allows passing any kind of data, altough JSON is the preferred exchange format since it allows verticles written in different languages to communicate, and more generally JSON is a popular general-purpose semi-structured data marshalling text format.
